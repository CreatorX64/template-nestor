...
After you've configured and made this template your own, it's time to build the project and publish it! To achieve building, run the following command:
...
- **template**: Includes source code of the template.
  - **public**: Includes images, favicons, fonts, manifest file, and other assets that Vite will serve from root address / (more on this later).
  ...
  - **package.json**: Includes a list of direct depencencies of the template project, some browser configuration, and executable commands that can be used to build the project or start the dev server (more on this later).
...
- **LICENSE.md**: Includes the license for this package.
...
- **Vite**: Used as a bundler, transformer, and a dev server. I used Vite instead of other bundlers because it's very intuitive to use and requires zero configuration (notice there isn't any config file for Vite in the project, because it's not needed for vanilla projects).
...
In the **index.html** file, other important notes are tagged with "_@NOTE_" comment. I recommend reading these comments before you make updates to the file as they can save you a lot of time. I wrote them in the file because they are concerned with specific class names or conventions in a specific location in the file. Again, to see the imporant notes, you can search for "_@NOTE_" in your text editor.

## Useful Notes: script.js

Sections in script.js are separated based on which part of index.html they are concerned with. That's why each section in script.js is separated into independent parts that is concerned with one section in index.html.
...
- Job Finder Ui App Kit by UI Freebies: Used inside the device mockup in Hero section. Note that you'll need to replace this image with your own app screenshot. Resource:
  https://www.figma.com/community/file/1095632577663801003/Job-Finder-Ui-App-Kit

- Testimonials photos (you can swap these with photos of your customers or their brand logos):
